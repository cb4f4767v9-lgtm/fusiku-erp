...
function normalizeLowerStepChannel(c: string | undefined): 'email' | 'sms' | 'whatsapp' {
  const x = String(c || 'email').toLowerCase();
  if (x === 'sms' || x === 'whatsapp') return x;
  return 'email';
}

function toApiStepUpChannel(c: string): 'EMAIL' | 'SMS' | 'WHATSAPP' {
...
export default function LoginPage() {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const out = await verifyOtp(challengeId, digits);
      ...
      <div className={`min-h-screen w-full flex items-center justify-center overflow-x-hidden overflow-y-auto ${isRTL ? 'direction-rtl' : ''}`}>
        <div className={`w-full max-w-6xl grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-16 items-center px-6 lg:px-10 py-10`}>
          ...
          <section className="flex items-center justify-center lg:justify-center">
            <div className="w-full max-w-[380px] p-8 rounded-2xl bg-white/10 border border-white/20 backdrop-blur-2xl shadow-xl">
              <div className="flex flex-col">
                <div className="flex flex-col gap-2">
                  {!challengeId ? (
                    <>
                      <div className="text-2xl font-semibold mb-2">{t('login.welcomeBack', { defaultValue: 'Welcome back' })} 👋</div>
                      <div className="text-sm text-white/70 mb-6">{t('login.subtitle', { defaultValue: 'Sign in with a one-time code' })}</div>
                    </>
                  ) : (
                    <>
                ...
                {challengeId ? (
                    ...
                    <div className="flex flex-col gap-4">
                      <label className="flex flex-col">
                        ...
                        <input
                          ...
                          maxLength={6}
                          className="w-full px-4 py-3 rounded-xl bg-white/10 border border-white/20 text-sm text-white placeholder:text-white/40 outline-none tracking-[0.35em]"
                          value={otpCode}
                      ...
                      <button
                        ...
                        disabled={loading}
                        className="w-full py-2 rounded-xl bg-white/10 border border-white/20 text-xs font-semibold text-white/85 disabled:opacity-50"
                      >
                  ...
                  <form id="otp-request" onSubmit={handleSendOtp}>
                    ...
                    >
                      {loading ? t('login.signingIn') : t('login.signin', { defaultValue: 'Send OTP' })}
                    </button>
