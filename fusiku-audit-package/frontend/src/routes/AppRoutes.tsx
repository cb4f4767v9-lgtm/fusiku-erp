...
function ProtectedRoute({ children }: { children: React.ReactNode }) {
  ...
  // Block dashboard access when trial expired (no active plan).
  // This avoids a brief "dashboard flash" before the first API call returns 402.
  useEffect(() => {
    let cancelled = false;
  ...
  if (checkingSaas) return <LoadingSkeleton variant="dashboard" />;
  return <>{children}</>;
}
