  ...
  return (
    <section className="section card dashboard-activity" aria-label={t('dashboard.recentSales')}>
      ...
      {recentSales.length ? (
        ...
        <EmptyState
          icon={<ClipboardList />}
          title={t('dashboard.noRecentSales', { defaultValue: 'No sales yet — let’s start your first one 🚀' })}
          description={t('dashboard.noRecentSalesHint', {
            defaultValue: 'Create your first sale in POS and we’ll show it here instantly.',
          })}
          action={
            <Link to="/pos" className="btn btn-primary">
              <DollarSign size={16} /> {t('dashboard.createFirstSale', { defaultValue: 'Create First Sale' })}
            </Link>
          }
        />
      )}
