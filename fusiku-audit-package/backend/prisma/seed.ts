...
// TEMP: limit models to avoid slow seed on remote DB (Railway)
// Later we will restore full dataset and optimize with bulk insert
const LIMITED_PHONE_MODELS = PHONE_MODELS.slice(0, 3);
...
const PERMISSIONS = [
  ...
  { code: 'institute.access', name: 'Institute module (students, courses, batches, fees, attendance)' },
  { code: 'parts.catalog.view', name: 'Parts catalog (device hierarchy)' },
  { code: 'parts.catalog.suggest', name: 'Submit parts catalog suggestions (AI / draft)' },
  { code: 'parts.catalog.manage', name: 'Approve parts catalog suggestions and publish rows' },
  { code: 'sourcing.requests.manage', name: 'Sourcing requests' },
];
...
async function main() {
  ...
  const staffCodes = new Set([
    ...
    'institute.access',
    'parts.catalog.view',
    'parts.catalog.suggest',
    'sourcing.requests.manage',
  ]);
  for (const perm of permissions) {
  ...
  for (const { brand, model, storages, colors } of LIMITED_PHONE_MODELS) {
  ...
  // Lightweight weekly plan (Phase 2026-05): used for UI cycle toggle / simulated billing.
  // Duration is enforced in Subscription rows (endDate), not on the plan itself.
  await prisma.subscriptionPlan.upsert({
    where: { name: 'Weekly' },
    update: {
      priceMonthly: 9,
