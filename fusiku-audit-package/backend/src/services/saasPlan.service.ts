...
export const saasPlanService = {
  ...
  async resolveDefaultSignupPlanId(tx: Prisma.TransactionClient): Promise<string> {
    ...
    // Bootstrap a minimal Free plan so new installs can sign up without a manual seed step.
    // Existing/seeded deployments remain unaffected because this only runs when no plan exists.
