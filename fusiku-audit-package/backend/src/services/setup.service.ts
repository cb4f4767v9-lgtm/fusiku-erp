...
export const setupService = {
  ...
  /**
   * Create a new tenant (company + branch) and assign the current authenticated user as admin.
   * Intended for SaaS onboarding flows where the user is authenticated but not yet tied to a tenant.
   */
  async initialSetupForUser(
    userId: string,
    data: { companyName: string; branchName: string; currency: string }
  ) {
    ...
    return prisma.$transaction(async (tx) => {
      ...
      const alreadyAssigned = String(user.companyId || '').trim();
      if (alreadyAssigned) {
        const err: any = new Error('User is already assigned to a company');
        err.statusCode = 400;
        throw err;
      }
