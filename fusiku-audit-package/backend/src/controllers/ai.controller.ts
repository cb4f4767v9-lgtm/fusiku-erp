...
export const aiController = {
  ...
  /**
   * AI assistants must never insert catalog rows directly — only enqueue `PartCatalogSuggestion` with source=AI.
   * Admin approval: POST `/api/v1/part-catalog/suggestions/:id/apply`.
   */
  async proposePartCatalog(req: AuthRequest, res: Response) {
    try {
      const companyId = req.user?.companyId;
      const userId = (req.user as any)?.userId;
      if (!companyId || !userId) return res.status(403).json({ error: 'Tenant context missing' });
      ...
      const row = await partCatalogSuggestionService.createSuggestion({
        companyId,
        source: PartCatalogSuggestionSource.AI,
        payload,
      ...
      res.status(201).json({
        ...
        queued: true,
        data: row,
        hint: 'Awaiting admin approval before catalog rows are created.',
      });
    } catch (e: any) {
      const msg = e?.issues ? JSON.stringify(e.issues) : e.message;
      res.status(400).json({ error: msg || 'Invalid catalog proposal' });
    }
  },
};
