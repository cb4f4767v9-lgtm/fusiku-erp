...
export const setupProfileController = {
  async upsert(req: Request, res: Response) {
    try {
      const { businessType, businessTypes, platform, sourcingCountries, sourcingOther, requirements } = req.body || {};
      ...
      if (businessTypes != null && (!Array.isArray(businessTypes) || businessTypes.length === 0)) {
        return res.status(400).json({ error: 'businessTypes must be a non-empty array when provided' });
      }
      const out = await setupProfileService.upsert({
        businessType: String(businessType),
        businessTypes: Array.isArray(businessTypes) ? businessTypes.map((s) => String(s)) : null,
