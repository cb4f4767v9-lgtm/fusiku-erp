...
/** Queues a catalog proposal only — never writes `PartCatalog*` rows until admin applies `/part-catalog/suggestions/:id/apply`. */
router.post(
  '/part-catalog-proposal',
  requirePermission('parts.catalog.suggest'),
  aiController.proposePartCatalog
);
