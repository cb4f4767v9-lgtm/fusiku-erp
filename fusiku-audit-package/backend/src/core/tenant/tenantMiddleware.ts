...
/** Routes that provision a tenant (no companyId yet) — Prisma must not enforce tenant isolation. */
const TENANT_ISOLATION_BYPASS_PREFIXES = ['/api/v1/signup'] as const;

function isTenantIsolationBypassPath(p?: string): boolean {
  if (!p) return false;
  return TENANT_ISOLATION_BYPASS_PREFIXES.some((prefix) => p.startsWith(prefix));
}
...
export const tenantIsolationMiddleware: Prisma.Middleware = async (params, next) => {
  ...
  // Signup and similar routes create Company/User without a prior companyId — bypass isolation entirely.
  if (isTenantIsolationBypassPath(ctx?.requestPath)) {
    return next(params);
  }
