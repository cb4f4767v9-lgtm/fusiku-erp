...
export const saasEnforcementMiddleware = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Trial expired and no active plan: force client to pricing.
    // We cannot issue a browser redirect from an API middleware reliably, so we return a stable 402 code
    // and the frontend routes to /pricing.
    const now = new Date();
    const trialExpired =
      overlay.subscription?.status === 'trial' &&
      !!overlay.subscription?.trialEndsAt &&
      new Date(overlay.subscription.trialEndsAt).getTime() < now.getTime();
    const noActivePlan = overlay.subscription?.status !== 'active';
    if (trialExpired && noActivePlan) {
      return res.status(402).json({
        error: 'Trial expired. Choose a plan to continue.',
        code: 'TRIAL_EXPIRED',
      });
    }
